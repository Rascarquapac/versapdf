 ...
 */
class pdf_limule extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->posxprogress=177; // Only displayed for situation invoices
		$this->postotalht=189;
		...
		{
			$this->postotalht=$this->posxprogress;
			$this->posxprogress=$this->posxdiscount;
			$this->posxdiscount=$this->posxunit;
			//$this->posxunit-=($this->posxunit - $this->posxqty);
		}
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		$outputlangs->load("sitfac@sitfac");
		...
		if ($conf->facture->dir_output)
		{
			...
			{
				$options = sizeof($invoice->array_options) ? $invoice->array_options : array();

				if (isset($options['options_rg']) && $options['options_rg'])
				{
					$this->is_rg = true;
				}
			}
	
			/*$nextinvoices = sizeof($object->tab_next_situation_invoice) ? $object->tab_next_situation_invoice : array();

			foreach ($nextinvoices as $invoice)
			{
				$options = sizeof($invoice->array_options) ? $invoice->array_options : array();
			...
			if (file_exists($dir))
			{
				...
				if (sizeof($propals))
				{
					...
					$label = $outputlangs->transnoentities("ProposalTotalHT");
					...
					// Output Rect
					$pdf->SetDrawColor(128,128,128);
					$this->printRect($pdf, $this->marge_gauche, $tab_top, $this->page_largeur-$this->marge_gauche-$this->marge_droite, 6);	// Rect prend une longueur en 3eme param et 4eme param	
				...
				if ($this->situationinvoice)
				{
					...
					$pdf->setTopMargin($tab_top_newpage);
					$pdf->setPageOrientation('', 1, $heightforfooter+$heightforfreetext+$heightforinfotot);	// The only function to edit the bottom 
					...
					foreach ($previousinvoices as $invoice)
					{
						...
						$pdf->MultiCell($this->page_largeur-($this->marge_droite+$this->marge_gauche), 3, $ref. ' '.$invoice->ref, 0, 'L', 0);
						...
						{
							$index++;
							$date_infos = dol_getdate($object->date, true);
							$date_rg = dol_mktime($date_infos['hours'], $date_infos['minutes'], $date_infos['seconds'], $date_infos['mon'], $date_infos['mday'], $date_infos['year']+1);


							$pdf->SetXY($posx, $posy + $height * $index);
							$pdf->SetTextColor(0,0,60);
							$pdf->SetFillColor(241,241,241);
							$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("ReductionWarrantyShort"), $useborder, 'L', 1);

							$total_rg = price2num(price($total_ttc * 0.05), 'MT');


							$pdf->SetXY($posx+$width, $posy + $height * $index);
							$pdf->MultiCell($width2, $height, price(-$sign * $total_rg, 0, $outputlangs), $useborder, 'R', 1);

							$total_with_rg_tc = $total_ttc - $total_rg;
							$index++;
							...
							$pdf->SetXY($posx+$width, $posy + $height * $index);
							$pdf->MultiCell($width2, $height, price($sign * $total_with_rg_tc, 0, $outputlangs), $useborder, 'R', 1);
						}
						...
						$resteapayer = price2num($invoice->total_ttc - $deja_regle - $total_rg - $creditnoteamount - $depositsamount, 'MT');
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			...
			else
			{
				...
				if ($this->is_rg)
				{
					...
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price(-$sign * $total_rg, 0, $outputlangs), $useborder, 'R', 1);

					$total_with_rg_ttc = $total_ttc - $total_rg;

					$index++;
					$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
					$pdf->SetTextColor(0,0,60);
					$pdf->SetFillColor(224,224,224);
					$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities("TotalWithRgTTC"), $useborder, 'L', 1);

					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($largcol2, $tab2_hl, price($sign * $total_with_rg_ttc, 0, $outputlangs), $useborder, 'R', 1);
		...
		$depositsamount=$object->getSumDepositsUsed();
		//print "x".$creditnoteamount."-".$depositsamount;exit;
		$resteapayer = price2num($object->total_ttc - $deja_regle - $total_rg - $creditnoteamount - $depositsamount, 'MT');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->posxtva-$this->posxdesc-1,2, $outputlangs->transnoentities("Designation"),'','L');
		}
		...
		if ($this->atleastonediscount)
		{
			...
			if (empty($hidetop))
			{
				...
				$pdf->SetXY($this->posxdiscount, $tab_top+1);
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->postotalht-1,2, $outputlangs->transnoentities("TotalHT"),'','C');
		}
	}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$outputlangs->load("propal");
		$outputlangs->load("companies");
		$outputlangs->load("sitfact@sitfac");
		...
		if ($this->situationinvoice) $title=$outputlangs->transnoentities("InvoiceSituationNumber", $object->situation_counter);
